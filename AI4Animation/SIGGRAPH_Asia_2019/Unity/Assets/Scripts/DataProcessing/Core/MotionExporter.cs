...
public class MotionExporter : EditorWindow {
	...
	private StreamWriter CreateFile(string name) {
		...
		if(!File.Exists(folder+name+".txt")) {
			...
			int i = 1;
			while(File.Exists(folder+name+"_("+i+").txt")) {
				i += 1;
			}
			filename = folder+name+"_("+i+")";
		}
    ...
    private IEnumerator ExportDataENVINTERACTION_ALL()
    {
        if (Editor == null)
        {
            Debug.Log("No editor found.");
    ...
    //// get root rotation vector
    //public float createYrotation(MotionEditor editor)
    //{
    //    Vector3 pos_leftshoulder = editor.GetActor().FindBone("mixamorig::LeftShoulder").Transform.position;
    //    Vector3 pos_rightshoulder = editor.GetActor().FindBone("mixamorig::RightShoulder").Transform.position;
    //    Vector3 pos_leftupleg = editor.GetActor().FindBone("mixamorig::LeftUpLeg").Transform.position;
    //    Vector3 pos_rightupleg = editor.GetActor().FindBone("mixamorig::RightUpLeg").Transform.position;
    //    //get x direction
    //    Vector3 vec_across0 = pos_leftshoulder - pos_rightshoulder;
    //    Vector3 vec_across1 = pos_leftupleg - pos_rightupleg;
    //    Vector3 vec_currentX = vec_across0 + vec_across1;
    //    vec_currentX.Normalize();
    //    // get foward direction
    //    Vector3 vec_forward = Vector3.Cross(vec_currentX, Vector3.up);
    //    vec_forward.Normalize();
    //    // assume Y angle
    //    float theta = Mathf.Acos(Vector3.Dot(vec_forward, Vector3.forward));
    //    if (vec_forward.x < 1e-3)
    //        theta = -1.0f * theta; // 좌표계 확인 필요

    //    return theta;
    //}
    public class InputEnvInteraction
    {
        public Frame Frame;
        public Matrix4x4 Root;
        public Matrix4x4 RootXZ;
