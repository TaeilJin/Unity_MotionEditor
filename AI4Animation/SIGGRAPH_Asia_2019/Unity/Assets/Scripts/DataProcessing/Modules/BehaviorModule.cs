...
public class BehaviorModule : Module {
	...
	protected override void DerivedInspector(MotionEditor editor) {
		...
		b_save = EditorGUILayout.Toggle("b_save", b_save);
		for(int i=0; i<Sensors.Length; i++) {
			EditorGUILayout.BeginHorizontal();
	...
	public class Sensor {
		...
		public BehaviorModule Module = null;
		public int Bone = 0;
		...
		public Sensor(BehaviorModule module, int bone, Vector3 offset, float threshold, float tolerance, float velocity, ID capture, ID edit) {
			Module = module;
			Bone = bone;
			Offset = offset;
			Threshold = threshold;
